	...
	if peer then
		if PDA.settings.acv == true then
			...
			function (page)
				...
				local pointer = 0
					while true do
						i = {string.find(page,"level of infamy. ]]></description>",pointer)}
						if i[2] ~= nil then
							log (string.sub(page,i[2]+23,i[2]+32))
...
log ("[PDA] Specialised MOD test started for player: "..(tostring(peer:name())))
	for i, mod in ipairs(peer:synced_mods()) do
		...
		elseif managers.job:current_level_id() == "red2" then
		specialmods = {"overdrill activator"}
		elseif managers.job:current_level_id() == "welcome_to_the_jungle_2" then
		specialmods = {"[big oil] engine helper"}
